...
namespace TraqCop.auth.Controllers
{
    public class AuthorizationController : Controller
    {
        ...
        public virtual async Task<IActionResult> Exchange()
        {
            ...
            if (request.IsAuthorizationCodeGrantType())
            {
                ...
                UserModel? user = await _userManager.GetUserAsync(authenticateResult.Principal);
        ...
        /// </summary>
        protected virtual async Task<UserModel> CreateUserFromExternalInfo(
        ...
        )
        {
            ...
            UserModel? user = await CreateNewUser(externalLoginInfo);
        ...
        protected virtual Task<UserModel?> CreateNewUser(ExternalLoginInfo externalUserInfo)
        {
            return Task.FromResult(new UserModel() { UserName = GetUserName(externalUserInfo), })!;
        }
        ...
        public virtual async Task<IActionResult> Authorize(string? provider)
        {
            ...
            try
            {
                UserModel user = await _userManager.FindByLoginAsync(
                     externalLoginInfo.LoginProvider,
                     externalLoginInfo.ProviderKey
                 );
        ...
        /// </summary>
        protected virtual async Task<IActionResult> SignInUser(UserModel user, OpenIddictRequest? request)
        {
            if (request == null)
            {
                throw new ArgumentNullException(nameof(request));
        ...
        /// </summary>
        protected virtual Task<IList<Claim>> GetClaims(UserModel user, OpenIddictRequest openIddictRequest)
        {
