...
namespace TraqCop.auth.Interface
{
      ...
      where TEntity : class
    {
        ...
        Task<IEnumerable<DTO>> Executebulk<DTO>(string sql, List<DynamicParameters> parameters, IDbTransaction transaction = null);
        Task<IEnumerable<DTO>> ExecuteStoredqueryProcedure<DTO>(string sql, DynamicParameters parameters, IDbTransaction transaction = null);
        DbDataReader ExecuteReader(CommandType cmdType, string cmdText);
        DbDataReader ExecuteReader(CommandType cmdType, string cmdText, DynamicParameters parameters);
